...
1. Create a Test JWT token (this is needed for tests to pass OK -- `go test`)
    ...
    TL;DR: Just copy and paste the following env vars in your system (`.env`)
        ...
        export IGN_FUEL_TEST_JWT=eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0LXVzZXItaWRlbnRpdHkifQ.iV59-kBkZ86XKKsph8fxEeyxDiswY1zvPGi4977cHbbDEkMA3Y3t_zzmwU4JEmjbTeToQZ_qFNJGGNufK2guLy0SAicwjDmv-3dHDfJUH5x1vfi1fZFnmX_b8215BNbCBZU0T2a9DEFypxAQCQyiAQDE9gS8anFLHHlbcWdJdGw
    ...
    1. The token-generator program will output a signed token. You will need to set the `IGN_FUEL_TEST_JWT` env var with that generated value.
...
1. Run the test suite
    ...
    ```
    export IGN_FUEL_DB_USERNAME=<DB username>
    ```

    ```
    export IGN_FUEL_DB_PASSWORD=<DB password>
    ```

    ```
    export IGN_FUEL_DB_ADDRESS=<DB IP and port. Eg: localhost:3306>
    ```
    ...
    ```
    export IGN_FUEL_TEST_JWT=<JWT token>
    ```

    ```
    export TEST_RSA256_PUBLIC_KEY=< RSA256 public key without the '-----BEGIN CERTIFICATE-----' and '-----END CERTIFICATE-----'>
    Note: TEST_RSA256_PUBLIC_KEY must be able to decode and validate the IGN_FUEL_TEST_JWT test token.
    ```
...
1. `IGN_FUEL_DB_USERNAME` : Mysql user name.
1. `IGN_FUEL_DB_PASSWORD` : Mysql user password.
1. `IGN_FUEL_DB_ADDRESS` : Mysql address, of the form `host:port`.
...
1. `IGN_FUEL_TEST_JWT` : A JWT token used from unit tests. Note: the `TEST_RSA256_PUBLIC_KEY` must be able to decode and validate this test token.
